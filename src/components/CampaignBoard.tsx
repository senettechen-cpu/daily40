...
export function CampaignBoard() {
    ...
    return <section className="planet-campaign" aria-label="黎明星球戰役">
        ...
        {captured === 3 ? <div className="campaign-victory" role="status">✦ 黎明守護者勳章已獲得<br /><small>三個據點全部收復。每一步努力，都留在這片領土上。</small></div> : <>
            ...
            <div className="campaign-tactics">{(Object.keys(TACTICS) as Tactic[]).filter(key => tacticAvailable(key, armyStrength)).map(key => <button key={key} aria-pressed={tactic === key} disabled={!tacticAvailable(key, armyStrength)} className={tactic === key ? 'selected' : ''} onClick={() => setTactic(key)}><strong>{TACTICS[key].name}</strong><span>{TACTICS[key].cost} 行動點</span><small>{TACTICS[key].description}</small></button>)}</div>
